...
**`about/index.html` and the legacy `about.html` redirect** may be edited only if explicitly instructed.
...
- `about/index.html` → `/shared/site.css` + `/styles.css`
- each project subpage → `/shared/site.css` + `/<project-slug>/<project-slug>.css`
- `photography/index.html` → `/shared/site.css` + `/photography.css`
...
Each project uses a lowercase kebab-case folder containing the page entry file and all project assets:
```
<project-slug>/
  index.html
  <project-slug>.css
...
Folders are named by **project**, not by number. There is no `work##` sequence anymore. The public URL is `/<project-slug>/`, without `.html`. Link the CSS as:
```html
<link href="/<project-slug>/<project-slug>.css" rel="stylesheet"/>
```

Existing root-level project `.html` files are compatibility redirects for old external links. Keep those redirects permanently so resumes, documents, bookmarks, and search results continue to work. The full page content belongs only in `<project-slug>/index.html`.
...
Canonical references: `bimtrieval/index.html` + `bimtrieval/`, and `neural-floorplan/index.html` + `neural-floorplan/`.
...
2. Create `<project-slug>/` and place `index.html` plus all project assets in it. Add `<project-slug>.css` (and `.js` if needed), building on the shared tokens.
3. In `<project-slug>/index.html`, load `/shared/site.css` + the project CSS + `/shared/site.js`, and follow the shared header/footer and case-study template (see the canonical references above).
