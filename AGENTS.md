...
Each new project consists of exactly two things:

**A. A clean-URL folder containing the page entry file:**
```
<projectname>/
  index.html
```
The folder name should be lowercase with no spaces; use hyphens when multiple words are needed. The public URL is `/<projectname>/`, without `.html`.
...
The `##` number is determined by sequence — always use the next number after the highest existing `work##` folder.
...
Existing root-level `<projectname>.html` files are compatibility redirects for old external links. Keep those redirects in place when editing migrated projects; the full page content belongs in `<projectname>/index.html`.
...
- `neural_floorplan/index.html` + `work13/work13.css`
- `explorentory/index.html` + its corresponding work folder CSS
...
4. Create `<projectname>/index.html`, following the style patterns of `neural_floorplan/index.html` and `explorentory/index.html`.
